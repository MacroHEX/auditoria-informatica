...
export function DisplayPanel() {
  ...
  const [lastCalled, setLastCalled] = useState<LlamadoTicket | null>(null);

  useEffect(() => {
    if (!socket) return;

    const handleInitialState = (data: { tickets: Ticket[]; ultimosLlamados: LlamadoTicket[] }) => {
  ...
  const waitingTickets = currentTickets.filter(t => t.estado === EstadoTicket.EnEspera);
  const calledTickets = currentTickets.filter(t => t.estado === EstadoTicket.Llamado);
  const completedTickets = currentTickets.filter(t => t.estado === EstadoTicket.Atendido);
